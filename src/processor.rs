...
//     // If no referer is provided, we use the host as a fallback
...
//     // MODERN BROWSER HANDSHAKE:
//     // CDNs look for these specific "Sec-Fetch" headers to verify browser requests
//     let request = agent.get(url)
//         .set("User-Agent", USER_AGENT)
//         .set("Referer", final_referer)
//         .set("Range", "bytes=0-0") // IDM Secret Sauce
//         .set("Accept", "*/*")
...
//             error_msg: Some(format!("HTTP Error {}: {}", url, e)) 
//         }),
//     };
...
// //             return (false, Some("Rejected: Server returned HTML (Likely a 403 error page)".to_string()));
// //         }
// //     }
// //     if size.unwrap_or(0) < 1024 && ct.is_none() {
// //         return (false, Some("Metadata missing or resource too small".to_string()));
// //     }
// //     (true, None)
// // }
...
//     // 2. STRICTER SIZE CHECK
//     // If < 5KB, it MUST be a specific small file type (like torrent). 
//     // If it claims to be a video/archive but is < 5KB, reject it.
//     if size_val < 5120 {
//         if ct.is_none() {
//             return (false, Some(format!("Rejected: Tiny file ({} bytes) with no content type", size_val)));
//         }
//         if let Some(c) = ct {
//             // A 200-byte MP4/ZIP is physically impossible (headers alone are larger)
//             if c.contains("video/") || c.contains("application/zip") || c.contains("application/x-rar") {
//                 return (false, Some(format!("Rejected: Fake Video/Archive detected (only {} bytes)", size_val)));
//             }
//         }
//     }
...
//     // Changes: https://tiktok.com/@user/video/123?is_from_webapp=1... 
//     // Into: https://www.tiktok.com/@user/video/123
//     let clean_url = if let Ok(mut parsed) = Url::parse(url) {
...
// fn process_instagram(url: &str, timeout: u64) -> Result<UrlInfo> {
//     let is_cdn = url.contains("fbcdn.net");
    
//     // 1. Extract Filename from URL (as we did before)
...
//     // 2. GET REAL SIZE: Make a HEAD request to get Content-Length
//     let agent = ureq::AgentBuilder::new()
...
//     // We only perform the HEAD check for direct CDN links to get the size
//     if is_cdn {
//         // MUST set Referer to instagram.com or the CDN returns 0 bytes/403
...
//         size, // This will now show the actual bytes instead of 0
//         content_type: Some(if is_cdn { "image/jpeg".into() } else { "video/mp4".into() }),
